...
function gameFirstGet(id, h) {
	...
	console.log("sendIp :"+sendIp+ "  playerid:  "+ playerid + "  game_id : " + thisGameId + " allGameId: " + gameid)
	$.ajax({
		type:'POST',
...
function gameGetMoney() {
	...
	$.ajax({
		type:'POST',
		url:sendIp + "games/" + gameid +"/game/reward",
		data:JSON.stringify(data),
		contentType:"application/json;charset=utf-8",
		success:function(res){
			console.log("======gameGetMoney  :" + JSON.stringify(res));
			if(res.success) {
				window.LjjsApp.showGetItem(res.data.itemPlus.FuCoins);
				localStorage.setItem("LuckCoinNum", res.data.fuCoins);
			}
...
function load_interactionExpressAd() {
	window.DDW.load_interactionExAd("945054605",600,400);
}

function play_interactionExpressAd() {
	if(window.DDW.play_interactionExAd(play_interactionEnd)){
		console.log('播放视频成功！');
	}else{
		alert("视频未缓存成功");
	}
	load_interactionExpressAd();
}

function play_interactionEnd(type,kind) {
...
!(function (global) {
	...
	var _showTime;
	function onShow(){
		for(let i =0;i< this.showFunList.length;i++) {
			this.showFunList[i]();
		}
		//alert('onShow');
		_showTime = Date.now();
	...
	function onHide(){
		for(let i =0;i< this.hideFunList.length;i++) {
			this.hideFunList[i]();
		}
		//alert('onhide');
	...
	window.DDW={
		...
		onShow:onShow,
		showFunList:[],
		regShowFun:function(cb){
			this.showFunList.push(cb);
		},
		unregShowFun:function(cb){
			let index = this.showFunList.indexOf(cb);
			if(index >= 0) {
				this.showFunList.splice(index, 1);
			}
		},
		hideFunList:[],
		regHideFun:function(cb){
			this.hideFunList.push(cb);
		},
		unregHideFun:function(cb){
			let index = this.hideFunList.indexOf(cb);
			if(index >= 0) {
				this.hideFunList.splice(index, 1);
			}
		},
		onHide:onHide,
		...
		_wxShareImg:function(code){
			if(this.m_WxShareImgCB){
				this.m_WxShareImgCB(code);
			}
		},

		_wxShareText:function(code){
			if(this.m_WxShareTextCB){
				this.m_WxShareTextCB(code);
			}
		},
		...
		wxShareImg:function(cb, url){
			this.m_WxShareImgCB=cb;
			this.callNative("wxShareImg",url);
		},

		wxShareText:function(cb, url){
			this.m_WxShareTextCB=cb;
			this.callNative("wxShareText",url);
		},
		...
		load_interactionExAd:function(codeid,w,h){
			this.callNative('load_interactionExpressAd',codeid,w,h);
		},
		...
		play_interactionExAd:function(){
			this.callNative('play_interactionExpressAd');
		},
		showSplash:function(){
...
(function (global) {
    var timeAll = 0;
    window.LjjsApp = {
        hideGetItem: hideGetItem,
        showGetItem: showGetItem,
        showTitleDialog: showTitleDialog,
        hideTitleDialog: hideTitleDialog,
        moreGame: moreGame
    }

    function showGetItem(num) {
        var obj = document.getElementById("getItemDialog");
		obj.className="divStyleGetGift1";
		$(obj).show();
		let num1 = document.getElementById("getItemNum");
		num1.innerHTML = num;
    }

    function hideGetItem() {
        var obj = document.getElementById("getItemDialog");
        $(obj).hide();
		obj.className="divStyleGetGift"
    }

    function showTitleDialog(time) {
        timeAll = time;
        var obj = document.getElementById("titleDialog");
        obj.className = "divStyleGetGift1";
        $(obj).show();
        var obj = document.getElementById("sureGetTime");
        obj.innerHTML = secondsFormat(timeAll);
    ...
    function secondsFormat(s) {
        ...
        let seconds = s % 60;
        let realh = hours.toString();
        let realm = minutes.toString();
        let reals = seconds.toString();
        if(hours < 10) {
        realh = "0"+hours;
        }
        if(minutes < 10) {
